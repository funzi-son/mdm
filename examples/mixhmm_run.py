...
EXP_DIR = HOME + "/WORK/experiments/multiresidential/DecTreePartition/1_21/"
...
def main():
    for data_id in range(1,2):#len(data)):
...
def run(state_id,data_id,alp,bet,gam):
    ...
    if LEAVE_ONE_OUT:
        result_dir = EXP_DIR + "LOO/" + models[0] + "/" + states[state_id] + "/" + data[data_id].__name__ + '/' + str(alp)+'_'+str(bet)+"_"+str(gam)
    else:
        result_dir = EXP_DIR + "model_select/" + models[0] + "/" + states[state_id] + "/" + data[data_id].__name__ + '/' + str(alp)+'_'+str(bet)+"_"+str(gam)
    ...
    if LEAVE_ONE_OUT:
        ...
        for fold in range(FOLD_NUM):
            ...
            hmm = MDM(dataset,alpha=alp,beta=bet,gamma=gam,state_type=states[state_id])
        ...
        hmm = MDM(dataset,alp,bet,gam,state_type=states[state_id])
