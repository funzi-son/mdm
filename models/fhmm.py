...
class FHMM_DIS(object):
    ...
    def __init__(self,dataset):
        ...
        self.tweights = []
        for i in range(rnum):
            self.tweights.append(np.zeros((hsize[i],hsize[i]),dtype=np.float))
        ...
        while True:
            ...
            if prev_act is not None:
                for i in range(self.rnum):
                        j_ = self.dataset.spr_act_map(prev_act[i],i)
                        j  = self.dataset.spr_act_map(curr_act[i],i)
                        self.tweights[i][j_,j] +=1
    ...
    def combine_log_params(self):
        ...
        for j_ in range(self.combined_act_num):
            for j in range(self.combined_act_num):
                prev = combined_acts[j_]
                curr = combined_acts[j]
                for r in range(self.rnum):
                    _tweights[j_,j] += self.tweights[r][self.dataset.spr_act_map(prev[r],r),self.dataset.spr_act_map(curr[r],r)]
...
class FHMM_VEC1(FHMM_VEC):
    def set_vec_type(self):
        self.vec_type = 1

class FHMM_VEC2(FHMM_VEC):
    def set_vec_type(self):
        self.vec_type = 2
        
class FHMM_VEC3(FHMM_VEC):
    def set_vec_type(self):
        self.vec_type = 3
